...
@tool
def get_security_findings(severity_filter: str = None) -> str:
    """Retrieve security findings from AWS services (severity: LOW, MEDIUM, HIGH, CRITICAL)"""
    try:
        result = security_tools.get_security_findings(severity_filter)
        return json.dumps(result, indent=2)
    except Exception as e:
