"""
Lambda bridge function: Bedrock Agent → AgentCore Gateway (CORRECT FLOW)
"""
import json
import requests
import base64
...
def lambda_handler(event, context):
    """Lambda handler that calls AgentCore Gateway"""
    ...
    try:
        ...
        if not token:
            return create_error_response(action_group, function_name, "Failed to obtain OAuth token")
        
        print("✅ OAuth token obtained successfully")
        
        # Map function name to tool name and parameters
        tool_name, mapped_params = map_function_to_tool(function_name, parameters)
        ...
        print(f"Calling AgentCore Gateway: {payload}")
        ...
        if response.status_code == 200:
            ...
            return create_error_response(action_group, function_name, f"Gateway error: {response.status_code}")
            
    except Exception as e:
        print(f"Exception: Lambda error: {str(e)}")
        import traceback
        traceback.print_exc()
        return create_error_response(action_group, function_name, str(e))

def map_function_to_tool(function_name, parameters):
    ...
    # Map function names to AgentCore tool names
    function_mapping = {
...
def create_success_response(action_group, function_name, result):
...
def create_error_response(action_group, function_name, error_message):
